...
if (!empty($_SESSION['winkelwagen']))
{
	...
	//dan laat hij die resultaten zien
      echo ("<tr><td>totaal bestelling</td><td></td><td></td><td>".$totaalbestelling."</td></tr></table>");

      echo ('<form name="eindbestel" action="'.$_SERVER['PHP_SELF'].'" method="POST"> <input type="submit" value="Verder winkelen" name="verderwinkelen"> <input type="submit" value="Plaats bestelling" name="plaatsbestelling"> </form>');
}
else{
       echo ("Uw winkelwagen is momenteel leeg.");
       echo ('<form name="leeg" action="'.$_SERVER['PHP_SELF'].'" method="POST"> <input type="submit" value="Verder winkelen" name="verderwinkelen"> </form>');
}
?>
